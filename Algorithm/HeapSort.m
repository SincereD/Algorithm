...
- (void)createBiggestHeap:(NSMutableArray *)list withSize:(int)size beIndex:(int)element{
    ...
    while (rchild < size) {//子树均在范围内
        ...
        if(list[lchild] <= list[rchild]){
            ...
            [self.sortRecord addObject:[self getRemoveRecordWithI:element J:lchild]];
            ...
            [self.sortRecord addObject:[self getRemoveRecordWithI:element J:rchild]];
    ...
    if (lchild < size && list[lchild] < list[element]) {
        ...
        [self.sortRecord addObject:[self getRemoveRecordWithI:lchild J:element]];
    }
}
...
- (void)heapSort:(NSMutableArray *)list{
    ...
    while (size > 0) {
        ...
        [self.sortRecord addObject:[self getRemoveRecordWithI:size - 1 J:0]];
